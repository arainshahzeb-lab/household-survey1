...
const EDU_OPTIONS = [
  "No Education","Primary (1-5)","Middle (6-8)","Matric (9-10)",
...
const OCCUPATION_OPTIONS = [
  "Farmer","Labor / Daily Wage","Government Job","Private Job",
...
export default function App() {
  ...
  const setMaleCount = (n) => setForm(f=>({
    ...
    maleChildEducations: Array.from({length:n},(_,i)=>f.maleChildEducations[i]??""),
  }));
  const setFemaleCount = (n) => setForm(f=>({
    ...f, femaleChildCount:n,
    femaleChildAges: Array.from({length:n},(_,i)=>f.femaleChildAges[i]??""),
    femaleChildEducations: Array.from({length:n},(_,i)=>f.femaleChildEducations[i]??""),
  }));
  ...
  const handleExport = () => {
    ...
    XLSX.writeFile(wb,`household_survey_${new Date().toISOString().slice(0,10)}.xlsx`);
